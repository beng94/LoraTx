...
// payload length
unsigned int payload_length = 6;

// tx packet
unsigned char txbuf[50] = "Monkey";
// rx packet
...
void SendData(String msg) {
  ...
  for(int i = 0; i < msg.length(); i++) {
      txbuf[i] = (unsigned char) msg.charAt(i);
    }
...
void loop() {
  ...
  while(1){
    ...
    Serial.println(msg.substring(0, msg.length()-1));
    
    SendData(msg.substring(0, msg.length()-1));
